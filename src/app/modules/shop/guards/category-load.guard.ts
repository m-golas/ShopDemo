...
export class CategoryLoadGuard implements CanActivate {
  ...
  canActivate(
    ...
    return this.areCategoriesLoaded().pipe(
      switchMap(() => this.hasProducts(id))
    );
  }
    ...
    return this.store.pipe(
      ...
      mergeMap(cached => {
        if(cached.includes(id)){
          return of(true).pipe(
            tap(() => this.store.dispatch(shopAction.categorySelected({categoryId: id})))
          );
        }
  ...
  loadProducts(id: number): Observable<boolean>{
    return this.shopService.getProductsByCategory(id).pipe(
      map(products => shopAction.productsLoadedAndCategorySelected({categoryId: id, products})),
  ...
  areCategoriesLoaded(): Observable<boolean> {
    return this.waitForCollectionToLoad().pipe(
      switchMap(() => this.hasCategories())
    );
  }

  waitForCollectionToLoad(): Observable<boolean> {
    return this.store.pipe(
      select(fromShop.isCategoryLoaded),
      take(1)
      )
  }
  ...
  hasCategoriesInStore(): Observable<boolean> {
    return this.store.pipe(
      select(fromShop.areCategoriesLoaded),
      take(1)
    )
  }
