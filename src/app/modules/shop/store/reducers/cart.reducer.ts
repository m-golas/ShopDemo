...
export const reducer = createReducer(
  ...
  on(shopAction.addProductToCart, (state, { product }) => addItem(state, product, doesExist(product, state))),

  on(shopAction.substractProductFromCart, (state, { product }) =>
    substractItem(state, product, doesExist(product, state))
  ),

  on(shopAction.removeProductFromCart, (state, { product }) => removeItem(state, product, doesExist(product, state))),

  on(shopAction.changeProductAmountinCart, (state, { product, amount }) =>
    changeAmount(state, product, amount, doesExist(product, state))
  ),

  on(shopAction.resetCart, () => initialState)
);
