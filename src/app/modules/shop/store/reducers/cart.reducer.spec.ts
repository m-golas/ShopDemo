...
describe('CartReducer', () => {
    ...
    const product: ShopProduct = {id: 2,displayName: 'Test',description: 'description', storeCategoryId: 2, price: 10};
    ...
    describe('substractProductFromCart action', () => {
        it('should return state if item doesnt exist', () => {
            ...
            const action = fromAction.substractProductFromCart({product: newProduct});
            const state = fromCart.reducer(snapState, action);
    ...
    describe('changeProductionAmountInCart action', () => {
        it('should return state if item doesnt exist', () => {
            const newProduct = {...product,id: 4}
            const action = fromAction.changeProductAmountinCart({product: newProduct, amount: 10});
            const state = fromCart.reducer(snapState, action);
        ...
        it('should change amount if item exist', () => {
            const action = fromAction.changeProductAmountinCart({product,amount: 10});
            const state = fromCart.reducer(snapState, action);
        ...
        it('should remove item if amount >= 0', () => {
            const { initialState } = fromCart;
            const action = fromAction.changeProductAmountinCart({product,amount: 0});
            const state = fromCart.reducer(snapState, action);
        ...
        it('should not let change amount above 99', () => {
            const action = fromAction.changeProductAmountinCart({product,amount: 100});
            const state = fromCart.reducer(snapState, action);
